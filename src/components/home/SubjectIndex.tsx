    ...
    <section aria-labelledby="subjects-title" className="pt-[clamp(1.5rem,1rem+3vw,3rem)] pb-[clamp(3rem,2rem+5vw,6.5rem)]">
      <div className="container-x grid gap-10 md:grid-cols-12 lg:grid-cols-12 lg:gap-8">
        ...
        <ol className="md:col-span-7 lg:col-span-7 md:col-start-6 lg:col-start-6">
          {categories.map((c, i) => (
            <li key={c.id} data-reveal style={revealDelay(Math.min(i, 6) * 60)} className="border-b border-line first:border-t">
              <Link href={`/courses?category=${c.slug}`} className="group flex items-baseline gap-4 py-4 sm:py-5">
                ...
                <span className="min-w-0 flex-1 font-display text-[1.125rem] font-semibold leading-tight text-ink transition-[color,transform] duration-200 ease-(--ease-editorial) group-hover:translate-x-2 group-hover:text-accent sm:text-[1.3125rem]">
                  {c.name}
                </span>
                <span className="hidden shrink-0 text-[0.8125rem] tabular-nums text-muted sm:block">{plural(c.courseCount, "course")}</span>
                <ArrowRight className="size-4 shrink-0 self-center text-muted transition-[color,transform] duration-200 ease-(--ease-editorial) group-hover:translate-x-1 group-hover:text-accent" />
              </Link>
            </li>
          ))}
        </ol>
